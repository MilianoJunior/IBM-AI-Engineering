...
class Data():
    ...
    def import_data(self):
        ...
        base = pd.read_csv('data/M4.csv', encoding=result['encoding'],low_memory=False)     
        
        base1 = pd.DataFrame(data=base[-self.num_days:-440].values,columns=base.columns)
    ...
    def training_assess(self,base,num_days = 565,colunas = ['Hora','dif', 'retacao +',
        ...
        colunas2 = ['Hora','retacao +','retracao -', 'RSI', 'M22M44','M66M44', 'ADX', 'ATR','Momentum',]
                                                            
        entrada_RNN = pd.DataFrame(data=base[-num_days:-440].values,columns=base.columns)      
        entrada_trade = pd.DataFrame(data=base[-num_days:-440].values,columns=base.columns)
